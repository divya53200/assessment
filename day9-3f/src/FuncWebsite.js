...
<h2>Our Tours</h2> 
    ...
    {datas.map((data, i) => <div key={i}><div className='b'><img src={data.image} /> 
        ...
        <br /> 
        <div> 
            <p >{read ? data.info : data.info.slice(0, 200)} 
            <button className='btn btn-secondary' onClick={handleToggle}>{read ? 'Show Less' : 'Read More'}</button> 
            </p> 
             
 
        </div> 
        <div ><button className='btn btn-danger' onClick={() =>handleDelete(i)}><center>Not Interested</center></button></div> 
